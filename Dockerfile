...
WORKDIR /zheong
RUN chmod -R 777 /zheong
RUN chmod -R 777 /usr/local

ARG uid
ARG user
ARG cuda
RUN useradd -d /zheong -u $uid $user 
...
USER $user
RUN mkdir -p /zheong/.mujoco \
    && wget https://www.roboti.us/download/mjpro150_linux.zip -O mujoco.zip --no-check-certificate \
    && unzip mujoco.zip -d /zheong/.mujoco \
    && rm mujoco.zip
RUN wget https://www.roboti.us/download/mujoco200_linux.zip -O mujoco.zip --no-check-certificate \
    && unzip mujoco.zip -d /zheong/.mujoco \
    && rm mujoco.zip
COPY ./mjkey.txt /zheong/.mujoco/mjkey.txt
ENV LD_LIBRARY_PATH /zheong/.mujoco/mjpro150/bin:${LD_LIBRARY_PATH}
ENV LD_LIBRARY_PATH /zheong/.mujoco/mjpro200_linux/bin:${LD_LIBRARY_PATH}
RUN pip3 install mujoco-py==1.50.1.68

RUN pip3 install git+https://github.com/CampagneLaboratory/torchfold@c2ac41b#egg=torchfold
RUN pip3 install baselines==0.1.5 --user

WORKDIR /zheong/amorpheus
ENV PYTHONPATH /zheong/amorpheus:/zheong/amorpheus/modular-rl/src:/zheong/amorpheus/modular-rl
